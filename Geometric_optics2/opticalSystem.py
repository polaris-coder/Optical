import math
from Geometric_optics2 import Material
import numpy as np
...
class LensSys():
    def __init__(self, OriginalLens, pupilRadius, pupiltheta, pupilPosition, wavelength):
        ...
        global WaveLength, L_OEP
        WaveLength = wavelength
        L_OEP = 0
        for i in range(self.pupilPosition):
            L_OEP = L_OEP + self.OriginalLens[i]['t']

        self.num_wl = len(WaveLength)
        self.num_lens = len(self.OriginalLens)
        self.Lens = []
        for nw in range(self.num_wl):
            lens = []
            for nl in range(len(OriginalLens)):
                if OriginalLens[nl]['m'] == 'vacuum' or OriginalLens[nl]['m'] == ' ':
                    n = 1.0
                else:
                    indexM = Material.m.index(OriginalLens[nl]['m'])
                    K1 = Material.K1[indexM]
                    L1 = Material.L1[indexM]
                    K2 = Material.K2[indexM]
                    L2 = Material.L2[indexM]
                    K3 = Material.K3[indexM]
                    L3 = Material.L3[indexM]
                    n = np.sqrt(K1*WaveLength[nw]**2/(WaveLength[nw]**2-L1) + K2*WaveLength[nw]**2/(WaveLength[nw]**2-L2)
                                + K3*WaveLength[nw]**2/(WaveLength[nw]**2-L3) + 1)
                lens.append({'C': OriginalLens[nl]['C'], 't': OriginalLens[nl]['t'], 'n': n})
    ...
    # 该部分用的是中间波长的透镜系统
    def calBfl(self):
        index_middle = round(self.num_wl / 2)
        Lens = self.Lens[index_middle - 1]
        ray = {'u': 0, 'y': self.pupilRadius}
        axial_rays = self.ParaxialRayTrace(ray, Lens)
        # 计算焦距
        bfl = -(axial_rays[self.num_lens - 3]['y'] / axial_rays[self.num_lens - 2]['u'])
        self.OriginalLens[self.num_lens - 2]['t'] = bfl
        for nw in range(self.num_wl):
            self.Lens[nw][self.num_lens - 2]['t'] = bfl

    # 功能: 计算入瞳中心光线在成像面上的坐标
    # 输入: theta: 视场角
    # 输出: 主光线在像面上的坐标
    def centralLocationInImage(self, thetas, Lens):
        thetas = thetas * math.pi / 180    # 角度转为弧度
        # 进行光线追迹
        ray = {'x': 0, 'y': -L_OEP * np.tan(thetas), 'z': 0, 'X': 0, 'Y': np.sin(thetas), 'Z': np.cos(thetas)}
        rays = self.SkewRayTrace(ray, Lens)
    ...
    # 输出: x    横坐标
    #      y    纵坐标
    def lateralAberration(self, thetas, apertureRays, num_phi, num_h):
        # 1. 初始化(光束信息, 包括：角度thetas，方向矢量[X, Y, Z])
        num_thetas = len(thetas)
        thetas = thetas * math.pi / 180    # 角度转弧度
        X = np.zeros(np.shape(thetas))
        Y = np.sin(thetas)
        Z = np.cos(thetas)    # 光线的方向矢量
        # 2. 对光线进行采样
        # 1) 对光线在入射面上进行采样
        ...
        # 4. 计算中心波长主光线与像面的交点坐标
        Lens = self.Lens[round(self.num_wl / 2) - 1]
        ray_middleWave = {'x': 0, 'y': -L_OEP * np.tan(thetas), 'z': 0, 'X': 0, 'Y': np.sin(thetas), 'Z': np.cos(thetas)}
        rays = self.SkewRayTrace(ray_middleWave, Lens)
        x0 = rays[self.num_lens - 1]['x']
        y0 = rays[self.num_lens - 1]['y']
        # 5. 计算艾里斑
        Dxp = self.pupilRadius * 2
        Zxp = 0
        for ns in range(self.pupilPosition, len(Lens)-1):
            Zxp = Zxp + Lens[ns]['t']
        AiryDiskRadius = 1.22 * WaveLength[round(self.num_wl / 2) - 1] * Zxp / Dxp * 0.001    # 艾里斑半径
        Athetas = np.linspace(0, 2 * np.pi, 100)
        a = AiryDiskRadius * np.cos(Athetas)
        b = AiryDiskRadius * np.sin(Athetas)
        AXY = {'X': a, 'Y': b}
        # 6. 计算不同波长的光束点列图的坐标
        XY = []
        for nw in range(self.num_wl):
    ...
    def longitudalAberration(self, thetas, apertureRays):
        ...
        # 计算成像面的位置
        imgs = np.array(range(-num_imgs_half, num_imgs_half)) * interval + self.OriginalLens[self.num_lens - 2]['t']
        ...
        for nw in range(self.num_wl):
            ...
            rays = self.SkewRayTrace(ray, Lens, 0)
            ...
            Error.append({'yc_error': z_yc_error, 'xc_error': z_xc_error})
    ...
    def distortion(self, thetas):
        ...
        for nw in range(len(WaveLength)):
            ...
            axial_rays = self.ParaxialRayTrace(axial_ray, Lens)
            # 任意斜光线追迹公式追迹光线
            skew_ray = {'x': 0, 'y': -L_OEP * np.tan(thetas), 'z': 0, 'X': 0, 'Y': np.sin(thetas), 'Z': np.cos(thetas)}
            skew_rays = self.SkewRayTrace(skew_ray, Lens)
            # 获取光线使用不同追迹公式在像面上的高度
            y1 = skew_rays[self.num_lens - 1]['y']     # 任意斜光线追迹
            y2 = axial_rays[self.num_lens - 1]['y']    # 近轴追迹
            # 计算畸变误差
            error = np.zeros(len(thetas))
            error[y2 != 0] = 100 * (y1[y2 != 0] - y2[y2 != 0]) / y2[y2 != 0]
            Distortion.append(error)
        return Distortion  # 返回畸变误差
    ...
    def ParaxialRayTrace(self, ray, Lens):
        axial_rays = []
        axial_rays.append(ray)
        for i in range(1, self.num_lens):
            u2, y2 = self.paraxial_raytrace(axial_rays[i - 1]['u'], axial_rays[i - 1]['y'], Lens[i - 1]['n'],
                                            Lens[i]['n'], Lens[i]['C'], Lens[i]['t'])
    ...
    def SkewRayTrace(self, ray, Lens, Oimg=1):
        rays = []
        rays.append(ray)
        if Oimg == 1:
            for i in range(1, self.num_lens):
